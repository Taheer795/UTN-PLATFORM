  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    filesArray.forEach(async (file) => {
      ...
      try {
        ...
        if (isImage) {
          console.log(`[File Change] Optimizing image before upload with canvas max width 2048px & 90% JPEG quality: ${file.name}`);
          finalFile = await resizeAndCompressImage(file);
        ...
        const finalPreview = isImage ? URL.createObjectURL(finalFile) : initialPreview;
        if (isImage) {
          try {
        ...
        if (isImage) {
          console.log(`[File Change] Initiating background pre-upload of optimized image to ImgBB for ${file.name}...`);
          let lastProgress = 0;
          uploadPromise = uploadToImgBB(finalFile, file.name, (statusProgress) => {
        ...
        uploadPromisesRef.current[itemId] = uploadPromise;

        // Await background completion
        const permanentUrl = await uploadPromise;

        // Await complete! Mark as ready and cache the direct URL
        setSelectedLocalFiles(prev => prev.map(item => {
          if (item.id === itemId) {
            return {
              ...item,
        ...
        console.log(`Background pre-upload success for ${(finalFile as any).name || file.name}: ${permanentUrl}`);
    ...
    try {
      ...
      if (selectedLocalFiles.length > 0) {
        ...
        for (let i = 0; i < selectedLocalFiles.length; i++) {
          ...
          if (isUrlRemote) {
            finalUrl = local.url!;
            console.log(`[Submission] Reusing already uploaded URL for ${local.file.name}: ${finalUrl}`);
          } else {
            if (type === 'image') {
              console.log(`[Submission] Resizing & uploading image to ImgBB: ${local.file.name}`);
              setNotification(`Processing & uploading image ${i + 1} of ${selectedLocalFiles.length} to ImgBB...`);
              
              // Compress to JPEG with 90% quality and scale to max 2048px width
              const optimizedBlob = await resizeAndCompressImage(local.file);
              finalUrl = await uploadToImgBB(optimizedBlob, local.file.name);
              console.log(`[Submission] ImgBB upload complete: ${finalUrl}`);
            } else {
              ...
              finalUrl = await uploadFileWithProgress(storagePath, local.file);
              console.log(`[Submission] Upload complete! Permanent download URL: ${finalUrl}`);
            }
          }
