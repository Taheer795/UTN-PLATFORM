...
/**
 * Generates an optimized, crystal-clear catalogue display version (approx 1200px on the longest side).
 * Preserves the original aspect ratio (never crops or forces to square).
 * Converts to WebP with JPEG fallback at 91% quality for sharp, high-speed catalogue browsing.
 */
export const generateOptimalThumbnail = (file: File): Promise<File> => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const supportsWebP = canvas.toDataURL('image/webp').indexOf('data:image/webp') === 0;
      const outputType = supportsWebP ? 'image/webp' : 'image/jpeg';
      const outputExt = supportsWebP ? '_display.webp' : '_display.jpg';
      const quality = 0.91; // 91% quality for crystal clear clarity on product details
      ...
      canvas.toBlob(
        (blob) => {
          ...
          if (blob) {
            ...
            console.log(`[UploadService] Generated sharp catalogue display image ${file.name} -> ${width}x${height} ${outputType} (${(compressedFile.size / 1024).toFixed(1)} KB)`);
            resolve(compressedFile);
          } else {
            resolve(file);
          }
...
export const generateCatalogueDisplayImage = generateOptimalThumbnail;
