...
export default function ListingCard({ listing, type, priority = false, onDelete, onEdit, onAction, onOrder }: ListingCardProps) {
                ...
                <AnimatePresence mode="wait">
                  {isJersey && currentMedia.url === defaultImage ? (
                    ...
                    <motion.img 
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      src={currentMedia.thumbnailUrl || currentMedia.url || defaultImage} 
                      ...
                      onError={() => {
                        if (currentMedia.thumbnailUrl && currentMedia.thumbnailUrl !== currentMedia.url) {
                          setCurrentMedia(prev => ({ ...prev, thumbnailUrl: undefined }));
                        } else {
                          setCurrentMedia({ url: defaultImage, type: 'image' });
                        }
                      }}
            ...
            <DialogContent className="max-w-4xl bg-slate-950 border border-slate-800 p-0 overflow-hidden rounded-[2rem] shadow-2xl flex flex-col max-h-[90vh]">
              <div className="relative flex-grow w-full flex items-center justify-center p-4 bg-black/40 min-h-0">
                {isJersey && currentMedia.url === defaultImage ? (
                  ...
                  <img 
                    src={currentMedia.url || currentMedia.thumbnailUrl || defaultImage} 
                    alt={listing.title || 'Product'}
